#!/bin/bash
# Script to create deployment script on DGX Spark
# Run this on your local machine, then transfer the command to DGX Spark

cat > /tmp/create_deploy.sh << 'DEPLOY_SCRIPT_EOF'
#!/bin/bash
...
echo "Script created at /tmp/create_deploy.sh"
echo "Copy this command to DGX Spark:"
echo ""
cat /tmp/create_deploy.sh

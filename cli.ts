...
program
  ...
  .action(async (sessionFile: string, options) => {
    try {
      ...
      const script = generator.generate(session);
      ...
      if (options.pageObject) {
        const poGenerator = new PageObjectGenerator();
        const pageObject = poGenerator.generate(session);
      ...
      console.log(`   - Elements: ${session.elements?.length || 0}`);
  ...
  .action(async (sessions: string[], options) => {
    try {
      ...
      if (merged) {
        ...
        console.log(`   Total elements: ${merged.elements.length}`);
      }
...
program
  ...
  .action(async (sessionFile: string) => {
    try {
      ...
      if (!session.name) warnings.push('Missing session name');
      ...
      if (!session.elements || !Array.isArray(session.elements)) {
        warnings.push('Missing elements array');
      }
      ...
      // Check elements
      session.elements?.forEach((element: any, index: number) => {
        if (!element.id) issues.push(`Element ${index}: Missing ID`);
        if (!element.locator) warnings.push(`Element ${index}: Missing locator`);
        if (element.locator && !element.locator.strategies?.length) {
          warnings.push(`Element ${index}: No locator strategies`);
        }
      });

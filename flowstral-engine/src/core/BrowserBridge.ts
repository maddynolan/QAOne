...
export abstract class BrowserBridge {
  ...
  protected registerDefaultHandlers(): void {
    ...
    this.on('applicationDetected', async (msg) => {
      ...
      this.sessionManager.updateSession({ application: fingerprint });
      return { fingerprint };
    });
    ...
    this.on('getStatus', async () => {
      ...
      // Status is tracked separately in SessionManager
      const isRecording = session !== null;
      return {
        isRecording,
        isPaused: false, // Would need to track separately
...
export class ExtensionBridge extends BrowserBridge {
  ...
  async connect(): Promise<void> {
    ...
    // Chrome extension API - check at runtime
    if (typeof (globalThis as any).chrome !== 'undefined' && (globalThis as any).chrome.runtime) {
      (globalThis as any).chrome.runtime.onConnect.addListener((port: any) => {
...
(function() {
  ...
  // Bridge to extension
  const port = (globalThis as any).chrome?.runtime?.connect({ name: 'flowstral' });
  let sessionId = null;
...
export class PostMessageBridge extends BrowserBridge {
  private targetWindow: any = null; // Window type - using any to avoid DOM dependency
  private targetOrigin: string = '*';

  constructor(
    engine: FlowstralEngine, 
    sessionManager: SessionManager, 
    targetWindow?: any, // Window type
    targetOrigin?: string,
    debug = false
  ) {
    super(engine, sessionManager, debug);
  ...
  async connect(): Promise<void> {
    if (typeof (globalThis as any).window !== 'undefined') {
      (globalThis as any).window.addEventListener('message', this.onMessage.bind(this));
    }
  ...
  disconnect(): void {
    if (typeof (globalThis as any).window !== 'undefined') {
      (globalThis as any).window.removeEventListener('message', this.onMessage.bind(this));
    }
  }
  ...
  send(message: BridgeMessage): void {
    const target = this.targetWindow || ((globalThis as any).window?.parent);
    target.postMessage({
      ...message,

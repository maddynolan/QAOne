  ...
  async findInShadow(selectors: string[]): Promise<any> {
    const element = await this.page.evaluateHandle((sels: string[]) => {
      function traverseShadow(root: Document | ShadowRoot, selector: string): Element | null {
        ...
        Array.from(allElements).forEach((el: Element) => {
          if (el.shadowRoot) {
            const found = traverseShadow(el.shadowRoot, selector);
      ...
      for (const sel of sels) {
        ...
        current = (found as Element).shadowRoot || (found as unknown as ShadowRoot);
      }
      return current;
    }, selectors);

...
10. [Lock Locators (Speed Optimization)](#lock-locators-speed-optimization)
...
│                                                                  │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │ LAYER 0: Locked Selector (if user locked)     ~99% instant  ││
│  │   optimizedSelector with 100ms timeout                      ││
│  │   🔒 Set via "Lock Locators" after successful test          ││
│  └─────────────────────────────────────────────────────────────┘│
│                        ▼ (if not locked or fails)                │
│  ┌─────────────────────────────────────────────────────────────┐│
...
## Lock Locators (Speed Optimization)

### Overview

After a successful test run, users can "Lock" the working selectors for instant playback.

### How to Use

1. Run test → All steps pass
2. Click "🔒 Lock Locators" button in the result modal
3. Save the test
4. Future runs use locked selectors (100ms timeout instead of full search)

### Technical Details

```javascript
// Locked selector stored in step
{
  selectorObj: {
    selector: "button.submit-btn",              // Original
    optimizedSelector: "button.submit-btn",     // Locked!
    optimizedAt: "2026-02-01T..."
  }
}

// Playback priority
1. Check optimizedSelector first (100ms timeout)
2. If found and visible → instant success
3. If not found → fallback to SmartFinder
```

### Benefits

| Without Locked | With Locked |
|----------------|-------------|
| 30-45 seconds | 5-10 seconds |
| Full 10-phase search per step | 100ms check per step |
| Learning happens locally | Works across all environments |

### Cross-Environment Support

Locked selectors travel WITH the test case, so they work:
- ✅ Local development
- ✅ CI/CD pipelines  
- ✅ Docker browser grids
- ✅ Remote Selenium/Playwright grids
...
|---------|------|---------|
| 2.2.0 | Feb 1, 2026 | Added Lock Locators feature, updated playback fallback chain |
| 2.1.0 | Jan 21, 2026 | Initial complete reference |

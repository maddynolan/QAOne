...
const PriorityBadge: React.FC<{ priority: string }> = ({ priority }) => {
  const colors: Record<string, string> = {
    ...
    low: 'bg-green-100 text-green-700 border-green-200 dark:bg-green-500/20 dark:text-green-400 dark:border-green-500/30',
  };
...
const AutomationBadge: React.FC<{ status: string }> = ({ status }) => {
  if (status === 'full') {
    return (
      <span className="flex items-center gap-1 px-2 py-0.5 text-xs font-medium rounded bg-emerald-100 text-emerald-700 border border-emerald-200 dark:bg-emerald-500/20 dark:text-emerald-400 dark:border-emerald-500/30">
  ...
  if (status === 'partial') {
    return (
      <span className="flex items-center gap-1 px-2 py-0.5 text-xs font-medium rounded bg-amber-100 text-amber-700 border border-amber-200 dark:bg-amber-500/20 dark:text-amber-400 dark:border-amber-500/30">
        <Bot className="w-3 h-3" />
        Partial
      </span>
    );
  ...
  return (
    <span className="flex items-center gap-1 px-2 py-0.5 text-xs font-medium rounded bg-gray-100 text-gray-600 border border-gray-200 dark:bg-gray-500/20 dark:text-gray-400 dark:border-gray-500/30">
...
const TestCaseRow: React.FC<TestCaseRowProps> = React.memo(({
  ...
  return (
    <div
      style={style}
      className={cn(
        'absolute top-0 left-0 w-full px-4 py-3 border-b',
        'border-gray-200 dark:border-gray-800/50',
        'flex items-center gap-4 cursor-pointer transition-colors',
        'hover:bg-gray-100 dark:hover:bg-gray-800/50',
        isSelected && 'bg-blue-50 dark:bg-amber-500/10 border-l-2 border-l-blue-600 dark:border-l-amber-500'
      )}
      onClick={onSelect}
    ...
    >
        ...
        {testCase.tags && testCase.tags.length > 0 && (
          <div className="flex items-center gap-1 mt-1">
            ...
            {testCase.tags.slice(0, 3).map((tag, i) => (
              <span key={i} className="text-xs text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded">
                {tag}
              </span>
            ))}
            {testCase.tags.length > 3 && (
              <span className="text-xs text-gray-500 dark:text-gray-600">+{testCase.tags.length - 3}</span>
            )}
          </div>
        )}
      </div>
      ...
      {/* Updated time */}
      {testCase.updated_at && (
        <div className="hidden xl:flex items-center gap-1 text-xs text-gray-600 dark:text-gray-400 flex-none">
      ...
      {/* Arrow */}
      <ChevronRight className="w-4 h-4 text-gray-400 dark:text-gray-600 flex-none" />
    </div>
  );
});
...
const SkeletonRow: React.FC<{ style: React.CSSProperties }> = ({ style }) => (
  <div
    style={style}
    className="absolute top-0 left-0 w-full px-4 py-3 border-b border-gray-200 dark:border-gray-800/50 flex items-center gap-4"
  >
    ...
    <div className="flex-1 space-y-2">
      ...
      <div className="h-3 bg-gray-100 dark:bg-gray-800/50 rounded w-2/3 animate-pulse" />
    </div>
...
export const VirtualTestCaseList: React.FC<VirtualTestCaseListProps> = ({
  ...
  if (!isLoading && testCases.length === 0) {
    return (
      <div className={cn('flex flex-col items-center justify-center h-64 text-gray-600 dark:text-gray-400', className)}>
        <AlertCircle className="w-12 h-12 mb-4 text-gray-400 dark:text-gray-600" />

      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button 
            ...
            size="sm"
            className={cn(
              "border-gray-700 text-gray-300 hover:bg-gray-800 h-9",
              priority && "border-amber-500/50 text-amber-400"
            )}
          >
        ...
        <DropdownMenuContent align="start" className="bg-gray-900 border-gray-700">
          <DropdownMenuItem onClick={() => onPriorityChange(null)} className="text-gray-300">
            All Priorities
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => onPriorityChange('critical')} className="text-red-400">
            Critical
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => onPriorityChange('high')} className="text-orange-400">
            High
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => onPriorityChange('medium')} className="text-yellow-400">
            Medium
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => onPriorityChange('low')} className="text-green-400">
            Low
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button 
            ...
            className={cn(
              "border-gray-700 text-gray-300 hover:bg-gray-800 h-9",
              automationStatus && "border-amber-500/50 text-amber-400"
            )}
          >
        ...
        <DropdownMenuContent align="start" className="bg-gray-900 border-gray-700">
          <DropdownMenuItem onClick={() => onAutomationStatusChange(null)} className="text-gray-300">
            All Statuses
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => onAutomationStatusChange('full')} className="text-emerald-400">
            <Bot className="w-4 h-4 mr-2" />
            Automated
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => onAutomationStatusChange('partial')} className="text-amber-400">
            <Bot className="w-4 h-4 mr-2" />
            Partial
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => onAutomationStatusChange('none')} className="text-gray-400">
    ...
    <div className="flex items-center justify-between px-4 py-2 bg-gray-900/50 border-b border-gray-800">
      ...
      <div className="flex items-center gap-4 text-sm">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-emerald-400">{automated.toLocaleString()}</span>
          <span className="text-gray-600">automated</span>
        </div>
        <div className="flex items-center gap-1.5">
          <FileText className="w-3.5 h-3.5 text-gray-400" />
          <span className="text-gray-400">{manual.toLocaleString()}</span>
